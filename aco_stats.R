...
for(t in 1:500) {
  ...
  for(i in 1:20) {
    control = list(maxiter=100, REPORT=0,TRACE=0, fn = subfitness, ants=5, reduction_rate=0.01, 
                   alpha=alpha, beta=beta, attach_extremes=FALSE)

    sr = aco(sol,fn=subfitness,change=twoopt,distance=dist_exp,lower=NA,upper=NA,control, type="min")
    
    cat(sr$eval, "\n")
    
    trace = updatetrace(trace, control$maxiter, "aco", "twoopt", "dist_exp", 
                        subfitness(sr$sol), NA, NA, NA, beta, alpha, init_eval, 
                        iterations=control$maxiter, subfitness(sr$sol), pmutation=NA, pcrossover=NA)
  }
