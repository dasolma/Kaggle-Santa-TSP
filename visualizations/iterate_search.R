...
while(TRUE) {
  ...
  if( algo == "hill" ) {
    ...
    result = hclimbing(sol, fitness, change=op[which(opnames==move)][[1]],
  ...
  if( algo == "sann") {
    ...
    result = sann(sol, fitness, change=op[which(opnames==move)][[1]],
                  distance=dist[which(distnames==move_control)][[1]],
  ...
  if( algo == "tabu") {
    ...
    result = tabu(sol, fitness, change=op[which(opnames==move)][[1]],
                  distance=dist[which(distnames==move_control)][[1]],
                  lower=rep(0,D), upper=rep(D,D), control=C)
